...
/*
 * NOTE for porting: shrink_slab() is mm/internal in mainline and is NOT
 * exported. This module relies on it being visible. On the OnePlus/MTK
 * tree, either (a) ensure EXPORT_SYMBOL(shrink_slab) exists in mm/vmscan.c,
 * or (b) switch the call below to an already-exported reclaim entry point.
 * The android_vh_shrink_slab_bypass vendor hook used to trigger this thread
 * is already present in include/trace/hooks/vmscan.h of this kernel.
 */
